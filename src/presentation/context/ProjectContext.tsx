...
import { setCookie, getCookie, COOKIE_NAMES } from '@/presentation/utils/cookies'
...
/**
 * Session storage key for persisting selected project (deprecated, using cookies now)
 */
...
export function ProjectProvider({ children }: ProjectProviderProps) {
  ...
  /**
   * Set selected project and persist to cookies
   */
  const setSelectedProject = (project: Project | null) => {
    setSelectedProjectState(project)

    // Persist to cookies (30 days expiration)
    if (project) {
      setCookie(COOKIE_NAMES.LAST_SELECTED_PROJECT, project.projectId, 30)
      ...
      setCookie(COOKIE_NAMES.LAST_SELECTED_PROJECT, '', -1) // Delete cookie
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      // Auto-select project if no project is selected
      if (activeProjects.length > 0 && !selectedProject) {
        // Try to restore from cookies first
        const savedProjectId = getCookie(COOKIE_NAMES.LAST_SELECTED_PROJECT)
        if (savedProjectId) {
          // Verify the saved project is still in the list
          const found = activeProjects.find((p) => p.projectId === savedProjectId)
          if (found) {
            setSelectedProjectState(found)
            console.log('✅ Restored selected project from cookie:', found.name)
          } else {
            // Saved project no longer accessible, select first
            console.log('⚠️ Saved project not found in accessible projects, selecting first')
          ...
          if (savedProject) {
            try {
              ...
              if (found) {
                ...
                setCookie(COOKIE_NAMES.LAST_SELECTED_PROJECT, found.projectId, 30)
                console.log('✅ Restored selected project from sessionStorage:', found.name)
              } else {
                setSelectedProject(activeProjects[0])
              }
            } catch {
            ...
            // No saved project, select first (random selection)
            console.log('📋 No saved project, auto-selecting first project')
            setSelectedProject(activeProjects[0])
          }
        }
      }
